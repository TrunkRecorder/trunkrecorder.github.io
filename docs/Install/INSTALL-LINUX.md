...
```bash
sudo apt-get install -y apt-transport-https build-essential ca-certificates ffmpeg git gnupg gnuradio gnuradio-dev gr-osmosdr libuhd-dev libboost-all-dev libcurl4-openssl-dev libgmp-dev libhackrf-dev liborc-0.4-dev libpthread-stubs0-dev libssl-dev libusb-dev pkg-config software-properties-common cmake libsndfile1-dev
```
...
```bash
sudo apt install git cmake make build-essential libboost-all-dev libusb-1.0-0.dev libaacs0 libcppunit-dev libcppunit-1.14-0 libssl-dev openssl curl ffmpeg libcurl3-gnutls libcurl4 libcurl4-openssl-dev
```
...
```bash
sudo pacman -S --needed base-devel git cmake boost gnuradio gnuradio-osmosdr libuhd ffmpeg
```
